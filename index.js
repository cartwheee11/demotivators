...
const TITLE_FONT =  TITLE_LINE_HEIGHT + 'px Times';
const SUBTITLE_LINE_HEIGHT = 30;
const SUBTITLE_FONT =  SUBTITLE_LINE_HEIGHT + 'px Arial';
	...
	if(words.length == 1) {
		...
		for(let i = 1; i < words.length; i++) {
			if(c.measureText(line + words[i] + ' ').width > width) {
				...
				c.fillText(line + words[i] + ' ', x, y + dy);
			} else {
				line = line + words[i] + ' ';
			}
		}
...
client.on('message', message => {
	...
	Canvas.loadImage(obtainedImageURL).then(obtainedImage => {
		...
		canvas.height = formatedImageHeight + CONTAINER_PADDING * 2 + textHegiht + TEXT_MARGIN_TOP;
		...
		c.font = SUBTITLE_FONT;
		printCanvasTextarea(c, subtitle, canvas.width / 2, CONTAINER_PADDING + formatedImageHeight + TEXT_MARGIN_TOP + titleHeight, CONTENT_WIDTH, SUBTITLE_LINE_HEIGHT);
